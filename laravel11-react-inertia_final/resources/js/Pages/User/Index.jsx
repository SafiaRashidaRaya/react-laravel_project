...
export default function Index({ auth, users, queryParams = null, success }) {
  ...
  // Modified part: Function to handle banning the user
  const banUser = (user) => {
    if (!window.confirm("Are you sure you want to ban the user?")) {
      return;
    }
    router.post(route("users.ban", user.id));
  };

  // Modified part: Function to handle unbanning the user
  const unbanUser = (user) => {
    if (!window.confirm("Are you sure you want to unban the user?")) {
      return;
    }
    router.post(route("users.unban", user.id));
  };
    ...
    >
      ...
      <div className="py-12">
        <div className="max-w-7xl mx-auto sm:px-6 lg:px-8">
          {success && (
            <div className="bg-emerald-500 dark:bg-emerald-500 py-2 px-4 text-white rounded">
              {success}
            </div>
          )}
                    ...
                    {users.data.map((user) => (
                      ...
                      >
                        ...
                        <td className="px-3 py-2 text-nowrap">
                          ...
                          {/* Modified part: Show ban or unban based on user status */}
                          {auth.user &&
                            auth.user.id !== user.id &&
                            (user.banned ? (
                              <button
                                onClick={() => unbanUser(user)} // Unban the user
                              ...
                              <button
                                onClick={() => banUser(user)} // Ban the user
